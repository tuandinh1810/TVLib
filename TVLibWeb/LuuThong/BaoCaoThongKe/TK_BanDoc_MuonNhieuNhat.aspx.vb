...
Partial Class BoSungBienMuc_ThongKe_MuonNhieuNhat
    ...
    Protected Sub LoadJs()
        btnThongKe.Attributes.Add("Onclick", "if((CheckDate(document.forms[0].ctl00$ContentPlaceHolder1$txtFromDate,'Không đúng kiểu định dạng ngày tháng!')== false) || (CheckDate(document.forms[0].ctl00$ContentPlaceHolder1$txtToDate,'Không đúng kiểu định dạng ngày tháng!')== false)){return false;}")
    End Sub
    ...
    Private Sub DrawTop20Stat()
        ...
        dtbThongKe = objTaiLieu.GetTop20_BanDoc(TVDateTime.ChuyenVietSangAnh(txtFromDate.Text.Trim()), TVDateTime.ChuyenVietSangAnh(txtToDate.Text.Trim()))
        ...
        If dtbThongKe.Rows.Count > 0 Then
            ...
            For intRowIndex = 0 To dtbThongKe.Rows.Count - 1
                ...
                If (dtbThongKe.Rows(intRowIndex)("SoThe").ToString().Length > 12) Then
                    arrLabel(intRowIndex) = dtbThongKe.Rows(intRowIndex)("SoThe").ToString().Substring(0, 12) & "..."
                Else
                    arrLabel(intRowIndex) = dtbThongKe.Rows(intRowIndex)("SoThe").ToString()
                End If

            Next
            strImage = Server.MapPath("../Reso0urces/Images/bground.gif")
            strVTitle = "Số lượt mượn"
            strHTitle1 = "Số thẻ bạn đọc"
            strHTitle2 = ""
