...
Partial Class BoSungBienMuc_ThongKe_BoSung
    ...
    Private Sub DrawTop20Stat()
        ...
        dtbThongKe = objTemp.RunSql("Select * from Kho")
        ...
        If dtbThongKe.Rows.Count > 0 Then
            ...
            For intRowIndex = 0 To dtbThongKe.Rows.Count - 1
                arrData(intRowIndex) = objTaiLieu.GetSoLuong_ByKho(txtFromDate.Text.Trim(), txToDate.Text.Trim(), CInt(dtbThongKe.Rows(intRowIndex).Item("KhoID")), 0).Rows(0)(0).ToString()
                arrLabel(intRowIndex) = dtbThongKe.Rows(intRowIndex).Item("MaKho")
            Next
        End If
        strImage = Server.MapPath("../Resources/Images/bground.gif")
        strVTitle = "Số lượng mã xếp giá"
        strHTitle1 = "Kho"
        strHTitle2 = ""
    ...
    Protected Sub btnXuatExcel_Click(sender As Object, e As EventArgs) Handles btnXuatExcel.Click
        ...
        End If
        dtbThongKe = objTaiLieu.GetSoLuongMXG_ByKho(dateTuNgay, dateDenNgay)


        ' *****************  Chuẩn bị tệp excel mẫu để ghi dữ liệu*******************
        Dim exBook As New Workbook()
        exBook.Open(Server.MapPath("~/Template/BaoCaoBoSung.xls"), FileFormatType.Excel2003)
        ...
        For i As Integer = 0 To dtbThongKe.Rows.Count - 1
            ...
            _range(DongHienTai, 1).PutValue(dtbThongKe.Rows(i)("TenKho").ToString())
        ...
        exBook.Save("BaoCaoBoSung" & DateTime.Now.ToString("dd_MM_yyyy") & ".xls", FileFormatType.Excel2003, SaveType.OpenInExcel, System.Web.HttpContext.Current.Response)
    End Sub
End Class